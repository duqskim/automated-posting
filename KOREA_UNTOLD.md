# Korea Untold — 콘텐츠 전략 플랜

> **The Real Story Behind the Drama**

---

## 시리즈 컨셉

| 항목 | 내용 |
|------|------|
| 채널명 | Korea Untold |
| 슬로건 | The Real Story Behind the Drama |
| 타겟 | 해외 K드라마/K팝 팬 (10~35세) |
| 언어 | 영어 (한국어 자막 병행) |
| 채널 | YouTube 메인 + Shorts 자동 추출 |
| 포맷 | 역사 스토리 → K드라마 팩트체크 삽입 |

---

## 타겟 오디언스

| 우선순위 | 타겟 | 심리적 욕구 |
|---------|------|------------|
| 1차 | K드라마 팬 (해외 여성 18-35세) | "K드라마에서 본 게 실제로도 있었나?" (검증 욕구) |
| 2차 | 한국 문화 관심 외국인 (Reddit r/korea, r/kdrama) | "한국 사람들이 이걸 어떻게 생각하나?" (내부자 관점) |
| 3차 | 해외 교포 2세 | "어렵지 않게 알고 싶다" (접근성) |

**→ 최적 아키타입: Explorer + Jester 혼합**
- "설명하는 사람"이 아니라 "같이 발견하는 사람"
- 유머로 심리적 진입 장벽을 낮춤

---

## 에피소드 아웃라인 (30편)

### Part 1 — Origins (고대~삼국)

| EP | 주제 | K드라마 연결 |
|----|------|-------------|
| 1 | 단군신화 — The Bear Who Became a Woman | - |
| 2 | 고조선 — Korea's First Kingdom (2333 BCE) | - |
| 3 | 고구려 — The Empire that Fought China | - |
| 4 | 백제 — The Kingdom that Civilized Japan | - |
| 5 | 신라 — The Women Warriors & Bone Ranks | - |
| 6 | 가야 — The Lost Kingdom | - |
| 7 | **삼국통일** — Why Silla Won (and the price it paid) | - |

---

### Part 2 — The Golden Age (고려)

| EP | 주제 | K드라마 연결 |
|----|------|-------------|
| 8 | 왕건 — The Merchant Who Became King | 태조 왕건 |
| 9 | 팔만대장경 — 80,000 Wooden Blocks vs. the Mongols | - |
| 10 | 몽골 침략 — 30 Years of Resistance | - |
| 11 | 공민왕 & 기황후 — The Queen Who Ruled the Yuan Empire | **기황후 팩트체크** |
| 12 | 고려 청자 — The Most Beautiful Pottery in the World | - |

---

### Part 3 — Joseon (조선 전기)

| EP | 주제 | K드라마 연결 |
|----|------|-------------|
| 13 | 이성계 — The Archer who Overthrew a Dynasty | **육룡이 나르샤 팩트체크** |
| 14 | 세종대왕 — The King who Invented an Alphabet | - |
| 15 | 장영실 — The Slave who Built Korea's First Clock | - |
| 16 | 조선의 여인들 — Women in Joseon: The Real Rules | **대장금 팩트체크** |
| 17 | 사화 — When Scholars Got Executed for Writing | - |
| 18 | 임진왜란 — The Turtle Ships vs. 200,000 Japanese Soldiers | - |
| 19 | 이순신 — The Admiral who Saved Korea (and died doing it) | - |

---

### Part 4 — Joseon (조선 후기~말기)

| EP | 주제 | K드라마 연결 |
|----|------|-------------|
| 20 | 광해군 — The King History Got Wrong | **광해, 왕이 된 남자 팩트체크** |
| 21 | 영조 & 사도세자 — A Father who Killed His Son | **옷소매 붉은 끝동 팩트체크** |
| 22 | 흥선대원군 — Korea's Last Strongman | - |
| 23 | 명성황후 — The Queen Murdered by Japan | **명성황후 팩트체크** |
| 24 | 구한말 — Korea at the Crossroads | **미스터 션샤인 팩트체크** |

---

### Part 5 — Modern Korea (근현대)

| EP | 주제 | K드라마 연결 |
|----|------|-------------|
| 25 | 3.1운동 — One Million People Said No | - |
| 26 | 독립운동가들 — The Fighters Nobody Talks About | - |
| 27 | 한국전쟁 — The War that Never Officially Ended | - |
| 28 | 한강의 기적 — From the Poorest to the Richest in 30 Years | - |
| 29 | 민주화 운동 — How Korea Built Its Democracy | - |
| 30 | K-Wave의 기원 — Why Korean Culture Went Global | - |

---

## Drama vs Reality — Shorts 별도 시리즈

3분 Shorts 시리즈. 드라마 장면 팩트체크 → 본편(EP N) 유입 CTA.

**구조:**
```
[00:00] 드라마 장면 설명 (대사/상황)
[00:20] "Here's what actually happened..."
[01:00] 역사적 팩트 + 시각 자료
[02:00] "The drama got [X] right / wrong because..."
[02:30] "Watch the full story → Episode N"
```

**예시 에피소드:**

| 드라마 | 팩트체크 주제 |
|--------|-------------|
| 대장금 | Was Jang Geum a real person? (실존 여부) |
| 기황후 | How powerful was Empress Ki really? |
| 킹덤 | Joseon's real disease outbreaks |
| 미스터 션샤인 | The real American-Korean soldiers |

---

## 캐릭터 전략

현재 제작 중인 캐릭터: **이시헌 (Lee Si-heon)** — 조선 시대 사관이 2024년으로 타임슬립

```
아키타입   : Explorer + Jester
포지션     : "한국 역사를 처음 배우는 외국인의 가이드"
특기       : K드라마 팩트체크 (역사적 고증 분석)
캐릭터 결함: 조선 선비가 현대 기술을 못 씀 → 공감/유머 요소
```

---

## 서비스 설계 & 구현 단계

### History Mode — 강화된 팩트체크

```python
# 역사 콘텐츠는 strict 모드 적용
HistoryFactCheckConfig:
    mode = "strict"
    warn_on_uncertain = True       # 불확실한 것도 경고
    required_checks = ["dates", "names", "events", "statistics"]
    # 연도 ±5년 이내 오차도 disputed 처리
```

### 구현 로드맵

| Phase | 내용 | 상태 |
|-------|------|------|
| Phase 1 | global(영어) 마켓 활성화 + EP1 테스트 | 예정 |
| Phase 2 | 시리즈 에피소드 DB + 일괄 생성 UI | 예정 |
| Phase 3 | Drama vs Reality 전용 포맷 + 일관된 비주얼 | 예정 |

### 즉시 시작 가능한 첫 에피소드

```
주제: "Dangun Myth — Korea's Origin Story Explained for Foreigners"
시장: US (영어)
플랫폼: YouTube + YouTube Shorts
캐릭터: 이시헌 (Lee Si-heon)
```

---

## 레퍼런스 채널 분석

| 채널 | 캐릭터 전략 | 참고 포인트 |
|------|------------|------------|
| Kurzgesagt | 추상적 새 캐릭터 | 인종/성별 없음 → 보편성 |
| Oversimplified | 못생긴 단순 사람 | 유머로 심리적 거리 낮춤 |
| Extra History | 역사 인물 삽화 | 시대 고증 느낌 + 현대 해석 |
| WaitButWhy | 막대기 인간 | 아이디어에 집중 |

**공통점:** 캐릭터가 "콘텐츠보다 앞서지 않는다" — 보조자 역할
